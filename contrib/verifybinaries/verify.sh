...
###   funexcoincore.org and funexcoin.org and compares them.
...
WORKINGDIR="/tmp/funexcoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://funexcoincore.org"
HOST2="https://funexcoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="funexcoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'funexcoin-',
...
if ! WGETOUT=$(wget -N -O "$SIGNATUREFILENAME.2" "$HOST2$BASEDIR$SIGNATUREFILENAME" 2>&1); then
   echo "funexcoin.org failed to provide signature file, but funexcoincore.org did?"
   echo "wget output:"
   # shellcheck disable=SC2001
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "funexcoin.org and funexcoincore.org signature files were not equal?"

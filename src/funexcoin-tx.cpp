...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " funexcoin-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  funexcoin-tx [options] <hex-tx> [commands]  Update hex-encoded funexcoin transaction\n" +
            "or:     funexcoin-tx [options] -create [commands]   Create hex-encoded funexcoin transaction\n" +
            "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded funexcoin transaction

...
ClientModel is used by the main application `funexcoingui` and several models like `peertablemodel`.
...
Used to process BIP21 payment URI requests. Also handles URI based application switching (e.g. when following a funexcoin:... link from a browser).
...
* `funexcoinstrings.cpp`: automatically generated
* `funexcoinunits.(h/cpp)`: FUNX / mFUNX / etc handling
* `callback.h`
...
* don't change `local/funexcoin_en.ts`; this happens [automatically](/doc/translation_process.md#writing-code-with-translations)
...
10. Start debugging with Qt Creator (you might need to the executable to "funexcoin-qt" under "Run", which is where you can also add command line arguments)

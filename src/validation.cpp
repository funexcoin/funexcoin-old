...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    if (!m_pool.CalculateMemPoolAncestors(*entry, setAncestors, m_limit_ancestors, m_limit_ancestor_size, m_limit_descendants, m_limit_descendant_size, errString)) {
        ...
        // this, see https://lists.linuxfoundation.org/pipermail/funexcoin-dev/2018-November/016518.html
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    {
       nSubsidy = 200000000 * COIN; // 200,000,000 Premine
    }   
    else if(nHeight<=1000) // next 999 Blocks
    {
        nSubsidy = 10000 * COIN; // 9,990,000
    }
    else if(nHeight<=75801200) // next 75800200 Blocks
    {
        nSubsidy = 50 * COIN; // 3,790,010,000
    }  
    else
    {
        nSubsidy = 0 * COIN;
